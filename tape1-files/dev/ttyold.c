...
/*                      rfg:bbn 81/04/27 various delays for Ann-Arbors */
/*			ers:bbn 81/08/11 in ttyinput, don't echo t_brkc */
/*					if it is ESCAPE */
...
register struct tty *tp;
{
	...
	if (t_flags&ECHO && (c != '\033' || c != tun.t_brkc)) {
		ttyoutput(c, tp);		/* ers(bbn) 11aug81 */
