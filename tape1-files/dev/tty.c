/*	tty.c	4.11	81/07/09	*/
/*              rfg:bbn 81/04/27 BBNTTNO tty queue empty call */
...
#ifdef BBNSYS
 	/* rfg:bbn	7/15/80		mod to determine whether
					tty char q's are empty */

	case BBNTTNO:
		t = 0;
		if (tp->t_canq.c_cc || tp->t_delct ||
			((tp->t_flags&(RAW|CBREAK)) && (tp->t_rawq.c_cc)))
			t = 1;
