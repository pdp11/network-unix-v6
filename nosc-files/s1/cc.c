...
Installation:
	...
	exit
: newer
	if ! { newer cc.c /bin/cc } exit
	echo s1/cc.c:
: finale
	cc -O -s -n cc.c
	if ! -r a.out exit
	if ! -r /usr/sys/s1/cc.c goto same
	if { cmp -s cc.c /usr/sys/s1/cc.c } goto same
	su cp cc.c /usr/sys/s1/cc.c
	su chmod 444 /usr/sys/s1/cc.c
	rm -f cc.c
: same
	if -r /bin/cc.c su rm -f /bin/cc
	su cp a.out /bin/cc
	rm -f a.out
	su chmod 555 /bin/cc
	su chown bin /bin/cc
...
Function:
	Supervisor for C compiler.

Compile time parameters and effects:
	SBSIZE -- The number of characters to allocate for use by define
	strings.

	SYMSIZ -- The maximum number of defines permitted.

Module History:
	Originally from Bell.
	Revised for SYMSIZ parameter Oct 77 by Greg Noel.
	Merged with DoD distribution version 13Mar79 by Greg Noel.  Comment
	    reproduced below.
	-L parameter added 14Mar79 by Greg Noel to specify library search
	    order for resolution of #include names which overrides default.
*/
#define	SBSIZE	2000
#define SYMSIZ	400
/* C command */
/*
 *      Modification to the preprocessor
 *      to allow persons to have a directory
 *      for use as a library for include files.
 *
 *      If the include file cannot be found,
 *      CC will search in the library directory
 *      for the file.  Modeled on the V7 C compiler.
 *      As in V7 the default directory is /usr/include.
 *
 *      Dennis L. Mumaugh, Dept of Defense, Fort George  G.  Meade
 *      18 April 1978
 */
		...
		out:
			...
			if (state==3) {
				...
				*lp = '\0';
				...
				ibuf2[0] = -1; fp = srchlst;
				do {
					for (sp = inclbf;
						(*sp++ = *fp++) != ':';);
					for (sp--, cp = filname;
						*sp++ = *cp++;);
				} while fopen(inclbf, ibuf2) < 0 && *fp;
				if (ibuf2[0] < 0)
