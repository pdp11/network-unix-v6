#/*
Module Name:
	init.c -- process control initialization
...
: final
	...
	if ! -r a.out exit
	su rm -f /etc/init
	su mv a.out /etc/init
	su chmod 555 /etc/init
	su chown root /etc/init
	su chgrp bin /etc/init

Synopsis:
	/etc/init

Function:
	Create a process for each login tty
	Also bunches of other stuff.
	See init (VIII) for details.

Module History:
	Written originally by someone at Bell Labs and included on their
	distribution tape.
