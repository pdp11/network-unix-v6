...
cloop:
	...
	while(c!='/' && c!='\0' && u.u_error==0) {
		if(cp < &u.u_dbuf[DIRSIZ])
			*cp++ = c;
		else {				/* RJB @ CAC 77-9-10 */
			u.u_error = EINVAL;	/* Tell him name too long. */
			goto out;		/* Not very informative */
						/* error number, but what */
		}				/* can one do? */
		c = (*func)();
	}
...
#endif UCBUFMOD
#ifdef UCBUFMOD
/* This is just a temporary mod until I figure out how to do
 * this efficiently... 
 */
	o = u.u_offset[1]&0777;
	d = &u.u_dent;
	for (i=0; i<(DIRSIZ+2)/2; i++)
		*d++ = fbword(bp->b_paddr, o+i);
#endif UCBUFMOD
