...
int bn;
{
	...
	if(i == 7) {
		i = (((bn>>8) & 0377) - 7)<<1;
		if((nb=fbword(bp->b_paddr,i)) == 0) {
			if((nbp = alloc(d)) == NULL) {
				brelse(bp);
				return(NULL);
			}
			sbword(bp->b_paddr,i,nbp->b_blkno);
			bdwrite(bp);
		} else {
			brelse(bp);
			nbp = bread(d, nb);
	...
	i = (bn & 0377)<<1;
	if((nb=fbword(bp->b_paddr,i)) == 0 && (nbp = alloc(d)) != NULL) {
		nb = nbp->b_blkno;
		sbword(bp->b_paddr,i,nb);
		bdwrite(nbp);
		bdwrite(bp);
	} else
		brelse(bp);
