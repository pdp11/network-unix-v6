 ...
 *
 * bug fix UNIX NEWS Vol 2, No 9, October 1977, p9-6.
...
breada(adev, blkno, rablkno)
{
	...
	if (rablkno && !incore(dev, rablkno)
	    && (bfreelist.av_forw != &bfreelist) ) {      /* bug fix */
...
#ifndef UCBUFMOD
	for (bp = &buf[0], i = 0; bp<&buf[NBUF]; bp++, i++) {
		bp->b_addr = buffers[i];
#endif UCBUFMOD
#ifdef UCBUFMOD
	for (bp = &buf[0]; bp<&buf[NBUF]; bp++) {
		bp->b_addr = (bp->b_paddr = malloc(coremap, 512/64))<<6;
...
#ifdef _______ /* JGN NOSC 1978Sep15 */
/*
...
#ifdef _______ /* RJB UoI 1977Mar13 */
/*
...
struct buf *abp;
{
#ifndef CPU40		/* if it isn't a /40 */
#ifndef CPU45		/* or a /45... */
		/* Eventually, it would be nice to get rid of all the
		   calls on these routines if it's not a /70 */
	register i, a;
	register struct buf *bp;

#ifndef CPU70		/* if we don't know what it is, include the test */
	if(cputype != 70)
		return;
#endif not CPU70
	spl6();
...
{
	register struct buf *swpbp;

#ifndef NSWBUF
	swpbp = &swbuf;
	spl6();
	while (swpbp->b_flags&B_BUSY) {
		swpbp->b_flags =| B_WANTED;
		sleep(&swpbp->b_flags, PSWP);
	}
#endif
#ifdef NSWBUF
	spl6();
	for(;;) {
		for (swpbp = &swbufs[0]; swpbp < &swbufs[NSWBUF]; swpbp++)
			if ((swpbp->b_flags&B_BUSY) == 0)
				goto gotbuf;
		swbufs[0].b_flags =| B_WANTED;
		sleep(&swbufs[0].b_flags, PSWP);
	}
gotbuf:
#endif NSWBUF
	swpbp->b_flags = B_BUSY | B_PHYS | rdflg;
	swpbp->b_dev = swapdev;
	swpbp->b_wcount = - (count<<5);	/* 32 w/block */
	swpbp->b_blkno = blkno;
	swpbp->b_addr = coreaddr<<6;	/* 64 b/block */
	swpbp->b_xmem = (coreaddr>>10) & 077;
	(*bdevsw[swapdev>>8].d_strategy)(swpbp);
	spl6();
	while((swpbp->b_flags&B_DONE)==0)
		sleep(&swpbp->b_flags, PSWP);
#ifndef NSWBUF
	if (swpbp->b_flags&B_WANTED)
		wakeup(&swpbp->b_flags);
#endif not NSWBUF
#ifdef NSWBUF
	if (swbufs[0].b_flags&B_WANTED) {
		swbufs[0].b_flags = ~B_WANTED;
		wakeup(&swbufs[0].b_flags);
	}
#endif NSWBUF
	spl0();
	swpbp->b_flags =& ~(B_BUSY|B_WANTED|B_PHYS);
	return(swpbp->b_flags&B_ERROR);
}
