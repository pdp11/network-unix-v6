...
history:
	...
	26Sep78 added call on rawbuf for raw messages jsq bbn
...
#ifdef RMI
	if( msgp && (msgp -> b_resv & b_raw)) {
		return ((bp = rawbuf()) ? linkbuf(bp, msgp) : 0);
	}
#endif RMI
...
int count;			/*number of bytes to move, if possible*/
{
...
#ifdef BUFMOD
	int offset;             /*holds offset during copy*/
#endif
	...
	while(*msgpaddr != 0 && count > 0)
	{
	    ...
	    {
#ifdef BUFMOD
		for(s=size;s>0;s--)     /*buffer to user*/
...
#ifdef BUFMOD
		for(offset=0;offset<s;)   /*shift data over in buffer*/
		    sbbyte(p->b_loc, offset++, fbbyte(p->b_loc, d++));
#endif BUFMOD
#ifndef BUFMOD
...
function:
	return a rathole for emergency use

algorithm:
	return the address of the rathole.

parameters:
	none

returns:
	address of the rathole

globals:
	address of the rathole

calls:
	for the address of the rathole.
...
emerbuf()
#ifdef BUFMOD
{return(&net_b.emerbuf[0]);}
#endif
#ifndef BUFMOD
{return(&net_b.emerbuf);}
#endif BUFMOD
...
history:
	...
	26Sep78 added raw buffer maintainance jsq bbn
*/
...
#ifdef RMI
	if (freedbuf -> b_resv & b_raw) {        /* if was used for rawmess */
		freedbuf -> b_resv =& ~b_raw;    /* turn off that flag */
		rbuf_count--;                   /* dec count of raw buffers */
		rawawk();
	}
#endif RMI
...
history:
	...
	Bug fixed by Bob Walton, LL - comment markers incorrect 28 July 78
...
freekb ()
{
    ...
    int found;		/* counter */
    int kb_index;
    char *newfreelist;		/* to point ot new freelist */

/* return;        /* JSK distributed it with this in- RLW */
    /*
    ...
    {
	for (kb_index = 0; kb_index < kb_hiwat; kb_index++)
	{
	    if ( (net_b.kb_[kb_index])
	    &&   (net_b.kb_[kb_index]->b_freect == net_b_per_k_b)  )
	    {
		/*
		 * find and free all the netbufs on this kbuf
		 * in the free list
		 */
...
#ifdef BUFMOD
		brelse(net_b.kb_[kb_index],&sbfreel);
#endif BUFMOD
#ifndef BUFMOD
...
#ifdef BUFMOD
	register struct netbuf *dbufp;
#endif BUFMOD
#ifndef BUFMOD
	register char *dbufp;		/*data part of a buffer pointer*/
#endif BUFMOD
...
#ifdef BUFMOD
	bufp = getsblk(NODEV,0);         /*get a kernel buffer. may sleep*/
#endif BUFMOD
#ifndef BUFMOD
...
#endif
#ifndef BUFMOD
		    dbufp =+ b_overhead + net_b_size;	/*to next buffer*/
#endif BUFMOD
		}
		net_b.b_cntfree =+ net_b_per_k_b;
...
history:
	...
	long hosts and long/short leaders jsq bbn 10-20-78
...
char  link;
{
	...
	if (sktp)
	{
		...
		firstbuf->b_len = BUFSIZE + impllen + hhllen + 1;
		ldrp->o_type = 0;
#ifndef SHORT
		ldrp -> o_nff = NFF;
		ldrp -> o_dnet = 0;
		ldrp -> o_lflgs = 0;
		ldrp -> o_htype = 0;
		ldrp -> o_host = host.h_hoi;
		ldrp -> o_imp = swab(host.h_imp);
#endif SHORT
#ifdef SHORT
		ldrp->o_host = ltoshost(host);
#endif SHORT
		ldrp->o_link = link;
		ldrp->o_subtype = 0;
		ldrp->o_pad1 = 0;
		ldrp->o_bsize = sktp->w_bsize;
		ldrp->o_bcnt = swab( ucnt/(ldrp->o_bsize>>3) );
		ldrp->o_pad2 = 0;
		bp->b_wcount = impllen + hhllen + 1;
	}
...
the stuff, besides being a more consistent mechanism.
*/
#ifdef BUFMOD
...
char  link;
{
	...
	if (sktp)
	{
		...
		netleader.o_lflgs = 0;
		netleader.o_htype = 0;
		netleader.o_host = host.h_hoi;
		netleader.o_imp = swab(host.h_imp);
#endif SHORT
#ifdef SHORT
		netleader.o_host = ltoshost(host);
#endif SHORT
		netleader.o_link = link;
		netleader.o_subtype = 0;
		netleader.o_pad1 = 0;
		if (link)
			netleader.o_bsize = sktp->w_bsize;
		else
			netleader.o_bsize = 8;
		netleader.o_bcnt = swab( ucnt/(netleader.o_bsize>>3) );
		netleader.o_pad2 = 0;

		vectomsg(&netleader, impllen+hhllen+1, &msg, 1);
	}
