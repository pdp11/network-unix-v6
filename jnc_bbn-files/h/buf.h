...
#ifdef BUFMOD
	char    *b_paddr;               /* physical address/64 of the buffer */
#endif BUFMOD
	char	*b_xmem;		/* high order core address */
	char	*b_blkno;		/* block # on device */
	char	b_error;		/* returned after I/O */
	char	*b_resid;		/* words not transferred after error */
	char	*b_proc;		/* process to be notified of i/o completion */
} buf[NBUF];

#ifdef BUFMOD
	/* control blocks of the NKS system buffers */

struct buf sbuf[NSBUF];
...
#ifdef BUFMOD
	char    *b_forw;                /* first buffer for this dev */
	char    *b_back;                /* last buffer for this dev */
	char    *d_actf;                /* head of I/O queue */
	char    *d_actl;                /* tail of I/O queue */
#endif BUFMOD
#ifndef BUFMOD
...
#ifdef BUFMOD
/* this is the head of the queue of available system buffers in user space */
...
#define B_SPEC	02000	/* special write from physio */
#ifdef PASYNC
#define B_PASYNC 04000  /* non-sleeping physio flag jsq BBN 2-26-79 */
#endif
#define BUFSIZE	20	/* number of bytes in buf for network interests */
