...
/*
 * Allocate size units from the given map as in malloc,
 * but only accept space from the highest free area.
 * This routine is (at the moment) exclusively used by set_lcba.
 */
#ifdef LCBA
mallhi(mp, size)
struct map *mp;
{
	...
	for (bp = mp; bp->m_size; bp++) ;
	--bp;
	if ((bp >= mp) && (bp->m_size >= size)) {
		bp -> m_size =- size;
		return (bp -> m_addr + bp -> m_size);
	}
	return(0);
}
#endif
