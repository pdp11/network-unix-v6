...
#define CREGS   0177740         /* 11/70 cache registers */

struct {
	int     loerad;         /* low error addr */
	int     hierad;         /* high error address */
	int     memerr;         /* error type */
	int     memctl;         /* memory control register */
};

#define FATAL   0160000         /* fatal errors */
...
trap(dev, sp, r1, nps, r0, pc, ps)
{
	...
	/* cache error -ugh rand:jrl */
	case 7:
	case USER+7:
		i = CREGS->memerr;      /* get error register */
		printf("address %o %o: %o\n",CREGS->hierad,CREGS->loerad,i);
		a = "RECOVERED - NO OPERATOR ACTION REQUIRED\n";
		if((CREGS->memerr&FATAL) == 0) {        /* soft err, ok */
			CREGS->memerr = i;
			printf(a);
			goto out;
		}
		if(dev&USER) {  /* if hard from user mode, just kill him */
			CREGS->memerr = i;
			printf(a);
			i = SIGKIL;
			break;
		}
	/* hard error from kernel, die */
	...
	case 6+USER: /* sys call */
		u.u_error = 0;
		ps =& ~EBIT;
		callp = &sysent[fuiword(pc-2)&0377];
		if (callp > &sysent[MAXSYSC])      /* no such system call */
			callp = &sysent[MAXSYSC];
		else
		if (callp == sysent) { /* indirect */
			a = fuiword(pc);
			pc =+ 2;
			i = fuword(a);
			if ((i&0377) > MAXSYSC)	/* fixed for non power of 2 */
				i = MAXSYSC;	/* by C.Howe, BBN 28Nov78 */
			callp = &sysent[i&0377];
