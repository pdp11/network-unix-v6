...
int *fp;
{
	...
	ip = rfp->f_inode;
	if(rfp->f_flag&FPIPE) {
		ip->i_mode =& ~(IREAD|IWRITE|IPIPE);	/*mek---turn off IPIPE */
		wakeup(ip+1);
		wakeup(ip+2);
		wakeup(ip+3);           /* rand:bobg--in case of eof's */
		awakei(ip,-1);		/* bbn:mek---interface to awake */
	}
	if(rfp->f_count <= 1) {
		if (rfp->f_flag&FEXCLU) /* rand:greep - exclusive open */
			ip->i_flag =& ~IEXCLU;
		/* Next 3 lines are Rand addition for ports - jsz, 3/76 */
		if (rfp->f_flag&FPORT)
			portclose(rfp);
		else
			closei(ip, rfp->f_flag&FWRITE);
	}
	rfp->f_count--;
}
...
int *ip;
{
	...
	plock(rip);             /* BBN:cdh improved locking */
	iput(rip);
}
...
int *aip;
{
	...
	ip = aip;
	m = mode;
	if(m == IEXOPN) {       /* rand:greep - test for exclusive open */
		if (ip->i_flag&IEXCLU) {
			u.u_error = ETXTBSY;
			return(1);
		}
