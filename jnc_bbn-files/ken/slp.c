...
setrun(p)
{
	...
	if( (rp = p) == 0 )  return;		/* return if param zero */
	rp->p_wchan = 0;
	rp->p_stat = SRUN;
	if(rp->p_pri < curpri)
		runrun++;
...
swtch()
{
	...
	if(p == NULL)
		p = &proc[0];
	/* remember current system time for interval timer usage
	 * When this process is resumed, this value is used to
	 * determine how long it has been dormant, and the
	 * interval timer is adjusted accordingly.
	 * (BBN:JFH 5/78)
	 */
	if (p->p_itima != NULL) {
		p->p_itime[0] = time[0];
...
loop:
	...
	retu(rp->p_addr);
	sureg();
	/* Adjust interval timer to account for elapsed time
	 * while process was not running
	 * (BBN:JFH 5/78)
	 */
	if (((i=rp->p_itima) != NULL) && ((i=fuword(i)) != NULL)) {
		n=dpcmp(time[0],time[1],rp->p_itime[0],rp->p_itime[1]);
		/* add seconds missed if necessary */
		if (n > 0) suword(i,(fuword(i)+n));
	}
	/*
...
newproc()
{
	 ...
	 */
retry:
	while( (rpp=cqdelink( &Free_proc )) == 0 );
	p = rpp;
	if( rpp > MAXPROC )
		MAXPROC = rpp;
	/*
	...
	up = rip;
	rpp->p_stat = SRUN;
	rpp->p_flag = SLOAD | (rip->p_flag & SNOSIG); /* propagate flag */
	rpp->p_uid = rip->p_uid;
	...
	rpp->p_time = 0;
	rpp->p_cpu = 0;
	rpp->p_sig = 0;         /* ill patch -- sigs for others get here */
	rpp->p_itima = 0;       /* BBN:JFH -- no initial interval timer */
