 ...
 *	alarm clock signals
 *	jab the scheduler
 *      increment current process' interval timer, if any
 */
...
out:
	if((ps&UMODE) == UMODE) {
		...
		if(++u.u_stime[1] == 0)
			++u.u_stime[0];
	pp = u.u_procp;
	dpadd(u.u_mem1,u.u_tsize + u.u_dsize + u.u_ssize); /* for accounting */
	if(++pp->p_cpu == 0)
		pp->p_cpu--;
	if(++lbolt >= HZ) {
		if((ps&0340) != 0)
		...
		/* increment current process' interval timer if any
		   (BBN:JFH 5/78) */
		if (((a = pp->p_itima) != NULL) &&
		    ((a = fuword(a)) != NULL)) suword(a,fuword(a)+1);
		spl1();
